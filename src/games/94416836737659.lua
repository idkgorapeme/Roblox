...
return function(section, data)
    ...
    -- noclip, so walls on the way cannot stop the flight
    ...
    local function stopNoclip()
        ...
        for _, d in ipairs(char:GetDescendants()) do
            if d:IsA("BasePart") and not d.CanCollide then
                -- only the ones we switched off get switched back on
                if d:GetAttribute("BPWasCollide") then
                    d.CanCollide = true
                    d:SetAttribute("BPWasCollide", nil)
                end
            end
        end
    end
        ...
        noclipConn = game:GetService("RunService").Stepped:Connect(function()
            ...
            for _, d in ipairs(char:GetDescendants()) do
                if d:IsA("BasePart") and d.CanCollide then
                    d:SetAttribute("BPWasCollide", true)
                    d.CanCollide = false
                end
            end
        end)
    ...
    elements:Toggle("Auto Farm", section, false, function(v)
        ...
        -- flight goes through walls, otherwise it gets stuck on the map
        startNoclip()
